...
#import "HoriImageManagerProtocol.h"

const CGFloat kHoriPhotoViewPadding = 10;
const CGFloat kHoriPhotoViewMaxScale = 3;

@interface HoriPhotoView ()<UIScrollViewDelegate, UIGestureRecognizerDelegate>

@property (nonatomic, strong, readwrite) UIImageView *imageView;
@property (nonatomic, strong, readwrite) HoriProgressLayer *progressLayer;
@property (nonatomic, strong, readwrite) HoriPhotoItem *item;
@property (nonatomic, strong) id<HoriImageManager> imageManager;

@end

@implementation HoriPhotoView

- (instancetype)initWithFrame:(CGRect)frame imageManager:(id<HoriImageManager>)imageManager {
    self = [super initWithFrame:frame];
    if (self) {
        self.bouncesZoom = YES;
        self.maximumZoomScale = kHoriPhotoViewMaxScale;
        ...
        _progressLayer = [[HoriProgressLayer alloc] initWithFrame:CGRectMake(0, 0, 40, 40)];
...
- (void)setItem:(HoriPhotoItem *)item determinate:(BOOL)determinate {
    ...
    if (item) {
        ...
        HoriImageManagerProgressBlock progressBlock = nil;
        if (determinate) {
...
- (void)resizeImageView {
    if (_imageView.image) {
        ...
        CGFloat width = self.frame.size.width - 2 * kHoriPhotoViewPadding;
